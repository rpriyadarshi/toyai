# Understanding Transformers: From First Principles to Mastery

**A Progressive Learning System with Hand-Calculable Examples**

---

## 📚 Book Structure

This book has been split into individual chapter files for easier navigation. 

**👉 Start here:** [`book/00-index.md`](book/00-index.md)

### Quick Links

**Part I: Foundations**
- [Chapter 1: Why Transformers?](book/01-why-transformers.md)
- [Chapter 2: The Matrix Core](book/02-matrix-core.md)
- [Chapter 3: Embeddings: Tokens to Vectors](book/03-embeddings.md)
- [Chapter 4: Attention Intuition](book/04-attention-intuition.md)

**Part II: Progressive Examples**
- [Example 1: Minimal Forward Pass](book/05-example1-forward-pass.md)
- [Example 2: Single Training Step](book/06-example2-single-step.md)
- [Example 3: Full Backpropagation](book/07-example3-full-backprop.md)
- [Example 4: Multiple Patterns](book/08-example4-multiple-patterns.md)
- [Example 5: Feed-Forward Layers](book/09-example5-feedforward.md)
- [Example 6: Complete Transformer](book/10-example6-complete.md)

**Appendices**
- [Appendix A: Matrix Calculus Reference](book/appendix-a-matrix-calculus.md)
- [Appendix B: Hand Calculation Tips](book/appendix-b-hand-calculation-tips.md)
- [Appendix C: Common Mistakes and Solutions](book/appendix-c-common-mistakes.md)

**Conclusion**
- [Conclusion](book/conclusion.md)

---

## 📖 How to Read

1. **Start with the index**: [`book/00-index.md`](book/00-index.md) for table of contents
2. **Read foundations first**: Chapters 1-4 establish core concepts
3. **Work through examples**: Examples 1-6 build progressively
4. **Use worksheets**: See `worksheets/` directory for hand-calculation guides
5. **Run code**: Build and run examples to see concepts in action

---

## 🗂️ File Organization

- **`book/`** - Individual chapter files (recommended for reading)
- **`BOOK.md`** - This file (index/redirect to chapter files)
- **`worksheets/`** - Hand-calculation guides for each example
- **`examples/`** - Working code implementations
- **`src/core/`** - Core library classes

---

**Note:** The book content is identical whether you read the individual chapter files or prefer a single document. The chapter files include navigation links for easier browsing.
