...
In this equation, $x$ represents the input, which is a vector of numbers. The variable $w$ represents the weights, which determine how important each input is. The bias $b$ provides a baseline offset that shifts the entire computation. The function $f()$ is called the activation function, and it shapes the output in a specific way. Finally, $y$ is the output—the decision made by the perceptron.
...
- **ReLU** (Rectified Linear Unit): $f(x) = \max(0, x)$ - This function keeps positive values unchanged and zeros out any negative values. It's the most commonly used activation in modern neural networks because it's simple, efficient, and works well in practice.

- **Sigmoid**: $f(x) = \frac{1}{1+e^{-x}}$ - This function squashes any input into the range 0 to 1, making it useful when you need outputs that represent probabilities.

- **Tanh**: $f(x) = \tanh(x)$ - Similar to sigmoid but squashes inputs into the range -1 to 1, providing a symmetric output around zero.
...
A layer is a group of neurons (perceptrons) that process information together. Think of a layer as a factory assembly line station: input arrives, the layer processes it, and output goes to the next layer. Each layer performs a specific job, and having multiple layers means multiple processing steps, each building on what the previous layer learned.
...
Layers enable complex transformations that a single perceptron cannot achieve. By stacking layers, networks can learn hierarchical patterns—simple patterns in early layers, and increasingly complex patterns in deeper layers. This hierarchical learning is what makes deep neural networks so powerful.
...
Here, $W_1$ is the first weight matrix that expands the dimensions, $b_1$ is the first bias vector, ReLU is the activation function, $W_2$ is the second weight matrix that compresses the dimensions back down, and $b_2$ is the second bias vector.

To understand this intuitively, think of a feed-forward network as a two-stage compressor. You start with a small package (a 2D vector, for example). In stage 1, you expand it into a large box (a 4D vector), giving you more room to work with the information. The activation function then filters and processes the contents. In stage 2, you compress it back down to a small package (back to 2D), but now it's been transformed in a meaningful way.
...
![Cross-Entropy Loss: L = -log(P(target))](images/cross-entropy-loss.svg)
...
In transformers, the forward pass follows a specific sequence of transformations. First, tokenization breaks the text into tokens. Then token encoding converts those tokens into integer IDs. Embedding lookup converts the integer IDs into vectors. Q/K/V projections transform those vectors into Query, Key, and Value vectors using weight matrices. Attention then combines these to produce a context vector. Output projection transforms the context vector into logits using the weight matrix WO. Finally, softmax converts the logits into probabilities.
...
![Training Progress: Loss Decreases Over Epochs](images/training-loss-epochs.svg)
...
**From Text to Numbers: Tokenization and Encoding**
...
**From Numbers to Vectors: Embeddings**
...
**Attention: Finding Relevance**

**Query (Q), Key (K), and Value (V)** are three different representations of the same token, each serving a specific purpose in attention. Think of attention like a library search system. The Query (Q) asks "What am I looking for?"—like typing a search query. The Key (K) says "What do I have to offer?"—like keywords on a book's spine. The Value (V) contains "What is my actual content?"—like the actual book content. **Q/K/V Maps** are three separate matrices that transform the same embedding into Query, Key, and Value vectors. Think of Q/K/V maps like three different lenses looking at the same object. You have the same token embedding (the object), but the WQ lens gives you the Query view (what I'm looking for), the WK lens gives you the Key view (what I'm advertising), and the WV lens gives you the Value view (what I actually contain).

The **attention dot product** is a mathematical operation that measures how similar two vectors are. Think of the dot product like measuring alignment. A high dot product means the vectors point in the same direction, indicating they're similar and relevant to each other. A low dot product means the vectors point in different directions, indicating they're different and not relevant.

**Making Predictions: Context, Output, and Probabilities**

A **context vector** is a weighted combination of all token values, where the weights come from attention. Think of a context vector like a blended smoothie. Each fruit (token value) contributes to the smoothie, and the amount of each fruit equals the attention weight. The final smoothie (the context vector) contains all the blended information. **Output Projection (WO)** is a matrix that transforms the context vector into vocabulary-space (predictions for each token). Think of WO like a translator that converts context meaning (in semantic space) into the likelihood of each word (in vocabulary space). It's like converting "animal, four legs, meows" into "cat: 80%, dog: 15%, mat: 5%".

**Logits** are the raw, unnormalized scores output by the model before applying softmax. Think of logits like raw test scores before grading on a curve. You might have raw scores [85, 90, 75, 80], and after applying the curve (softmax), you get probabilities [0.2, 0.5, 0.1, 0.2]. Logits can be any real numbers (positive, negative, large, small), while probabilities must be between 0 and 1 and sum to 1. **Softmax** is a function that converts numbers into probabilities (they sum to 1.0). Think of softmax like dividing a pie. If you have scores [5, 2, 1], softmax converts them to probabilities [0.7, 0.2, 0.1]. The largest score gets the biggest slice, and all slices sum to 1.0 (the whole pie).

**Training Techniques: Normalization and Residuals**

**Layer normalization** is a technique that normalizes the inputs to a layer by adjusting the mean and variance. Think of layer normalization like standardizing test scores. Raw scores might vary widely (0-100), but after normalization (subtract mean, divide by standard deviation), the scores are centered around 0 with a consistent scale. A **residual connection** adds the input of a layer directly to its output. Think of a residual connection like a shortcut or bypass. The main path goes Input → Layer → Output, but there's also a shortcut that goes Input → (directly to output). The final output is Layer(Input) + Input. Residual connections enable training of very deep networks by allowing gradients to flow directly through the shortcut, which helps prevent the vanishing gradient problem that can occur in very deep networks.
